method com.galois.ecc.P384ECC64.ec_mul
{
  ...
  from pc 80 {
    ...
    let auxres = ref_ec_mul_aux( { x = join(valueOf(r.x))
                                 ; y = join(valueOf(r.y))
                                 ; z = join(valueOf(r.z))
                                 }
                               , { x = join(valueOf(s.x))
   ...
   ensure valueOf(r.x) := split(auxres.x) : [12][32];
   ensure valueOf(r.y) := split(auxres.y) : [12][32];
   ensure valueOf(r.z) := split(auxres.z) : [12][32];
