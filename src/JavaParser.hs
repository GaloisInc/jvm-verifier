...
-- import Debug.Trace
  ...
  parseTypeDescriptor ('[' : rest)
    = let (tp, result) = parseTypeDescriptor rest
      in (ArrayType tp, result)
  ...
  makeMethodKey :: String -- ^ Method name
                ...
                -> MethodKey
  makeMethodKey name descriptor
    = let (returnType, parameters) = parseMethodDescriptor descriptor
      in MethodKey name parameters returnType
