...
step (BadInsn insn) =
  err . render $ "step: unexpected instruction" <+> text (ppInst insn)
...
evalCond (CompareRef cmpTy) = do
  ...
  case cmpTy of
    EQ -> rEq x y
    NE -> bNot =<< rEq x y
    _  -> err "invalid reference comparison; bug in symbolic translation?"
...
instance MonadSim sbe m => JavaSemantics (Simulator sbe m) where
  ...
  initializeClass name = do
    ...
    case M.lookup name (m^.memInitialization) of
      ...
      Nothing | otherwise -> do
        ...
        dbugM' 5 . render $ "initializing class" <+> text (unClassName name)
        cl <- lookupClass name
  ...
  throw ref@(Ref _ (ClassType cName)) = do
    ...
    errorPath $ FailRsn (unClassName cName)

  throw _ = err "throw: null or non-reference type thrown"
  ...
  doubleFromInt  i = do
    ...
    case asInt sbe i of
      ...
      Nothing -> err "cannot convert symbolic int to double"
  floatFromInt i = do
    sbe <- use backend
    case asInt sbe i of
      Just n -> return (fromIntegral n)
      Nothing -> err "cannot convert symbolic int to float"
  doubleFromLong l = do
    sbe <- use backend
    case asLong sbe l of
      Just n -> return (fromIntegral n)
      Nothing -> err "cannot convert symbolic long to double"
  floatFromLong  l = do
    sbe <- use backend
    case asLong sbe l of
      Just n -> return $ fromIntegral n
      Nothing -> err "cannot convert symbolic long to float"
  ...
  setArrayValue r idx (RValue v) = do
    ...
    case asInt sbe idx of
      Nothing -> err "Cannot update reference arrays at symbolic indices"
      Just i -> do
  ...
  setArrayValue _ _ val = do
    ...
    err "setArrayValue: unsupported array element type"
    ...
    case M.lookup (r, fieldId) (m^.memInstanceFields) of
      ...
      Nothing    -> do
        when (isFloatType (fieldIdType fieldId)) $
          err $ "internal: unassigned floating-point instance field "
  ...
  pushStaticFieldValue fieldId = do
    ...
    case M.lookup fieldId (m^.memStaticFields) of
      ...
      Nothing    -> do
        when (isFloatType (fieldIdType fieldId)) $
          err $ "internal: unassigned static floating-point field " ++
                        show fieldId
  ...
  getLocal i = do
    ...
    case mv of
      Just v -> return v
      Nothing -> err $ "internal: undefined local variable " ++ show i
...
{-
  ...
  dynBind' clName key objectRef cgen = do
    ...
    cls <- case mty of
             ...
             Just _ -> err "dynBind' type parameter not ClassType-constructed"
    let cases = (isNull objectRef |-> throwNullPtrExc) : map cgen cls
    -- In theory, this error should be unreachable.
    choice cases (err $ "Uncaught linker error: " ++ clName ++ ":" ++ show key)
  ...
  invokeStaticMethod cName key args = do
    ...
    case mapMaybe (\cl -> (,) cl `fmap` (cl `lookupMethod` key)) sups of
      ...
      [] -> err $ "Could not find static method " ++ show key ++ " in " ++ cName
...
drefString strRef = do
  ...
  case mapMaybe lkup fldIds of
    ...
    _ -> err "Invalid field name/type for java.lang.String instance"
-}
...
overrideInstanceMethod :: ClassName
                       -> MethodKey
...
overrideInstanceMethod cName key action = do
  ...
  when (k `M.member` overrides) $ do
    err $ "Method " ++ unClassName cName ++ "." ++ methodKeyName key  ++ " is already overridden."
...
overrideStaticMethod cName mKey action = do
  ...
  when (key `M.member` overrides) $
    abort $ "Method " ++ unClassName cName ++ "." ++ methodKeyName mKey ++ " is already overridden."
      ...
      , \this _ -> do
          ...
          pushValue =<< RValue
                        <$> if classNameIsArray' nm
                            then getClassObject (mkClassName (tail nm)) -- BH: why tail?
                            else return NullRef
      )
...
newClass :: MonadSim sbe m => ClassName -> Simulator sbe m Ref
newClass cName = do
  ...
  str <- newString (unClassName cName)
...
getClassName _ = err "getClassName: wrong argument type"
...
dynBind :: MonadSim sbe m
        => ClassName        -- ^ Name of 'this''s class
        -> MethodKey        -- ^ Key of method to invoke
        -> JSRef (Simulator sbe m) -- ^ 'this'
        -> (ClassName -> Simulator sbe m ()) -- ^ e.g., an invokeInstanceMethod invocation.
        -> Simulator sbe m ()
dynBind clName key objectRef f = do
  ...
  case impls of
    ...
    _    -> err . render $ "no implementations found for"
              <+> text (unClassName clName) <+> ppMethodKey key
...
dynBind' :: MonadSim sbe m
         ...
         -> MethodKey
         -> JSRef (Simulator sbe m)
         -> Simulator sbe m [ClassName]
dynBind' clName key objectRef = do
  dbugM' 6 . render $ "dynBind" <+> text (unClassName clName) <+> ppMethodKey key
  mty <- typeOf objectRef
  case mty of
    Nothing -> err . render $ "dynBind': could not determine type of reference"
    Just (ClassType instTy) -> do
      cb <- use codebase
      impls <- liftIO $ findVirtualMethodsByRef cb clName key instTy
      dbugM' 6 . render $ "found impls:" <+> sep (map (text . unClassName) impls)
      return impls
    Just _ -> err "dynBind' type parameter not ClassType-constructed"
...
toggleBreakpoint fn clName key bp = do
  ...
  let methodDoc = text (unClassName clName) <> "." <> ppMethodKey key
  method <- case lookupMethod cl key of
              Nothing -> err . render $ "unknown method:" <+> methodDoc
              Just m -> return m
